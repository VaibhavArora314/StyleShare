...
const ReactionButton: React.FC<ReactionButtonProps> = ({ postId, initialReaction }) => {
  ...
  return (
    ...
    >
      ...
      <br/>
      {reaction && (
        <div className="mt-2 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded  bg-white  dark:bg-[#000435] backdrop-blur-sm dark:text-blue-300  border border-sky-500">You reacted with {reaction}</div>
      )}
    </div>
  );
};
